...
try {
  ...
  api.on("exit", (code, signal) => {
    if (!shuttingDown) {
      log("api-crashed-preview-kept-alive", {
        runtimeId,
        childPid: api.pid,
        code,
        signal,
...
async function waitForHealth(
  ...
  while (Date.now() < deadline) {
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, Math.min(75 * attempt, 500)),
    );
  }
...
async function shutdown(signal: NodeJS.Signals, exitCode: number) {
  ...
  try {
    ...
    if (manifest.runtimeId === runtimeId && manifest.pid === process.pid)
...
async function fetchHealth(
  ...
  try {
    ...
    return health.app === "codex-poker" && health.ready ? health : undefined;
  } catch {
    return undefined;
  }
}
