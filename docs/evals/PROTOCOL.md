...
## Current status
Independent first-use UX and one-turn skill evaluations completed; ten-hand fresh-context native play evaluation completed with 24 model-chosen actions and nine deals. Reports retain their original observations and limits. Fable consultation completed through the native Claude app and is explicitly not blinded. Builder retested the mobile/history fixes when the independent evaluator lost browser exposure.

The ten-hand run exposed the 20-second host timeout collision. After shortening the wait to 15 seconds, the builder verified a native no-change response in 15,044 ms (`ok: true`, `changed: false`) and tested the production build through a local proxy that returns 503 for `/events`: human Call remained enabled, Codex acted through WebMCP, and HTTP polling restored the human Check control without an event stream. These are post-fix builder checks, not additional blinded runs.
