...
2. Call `act` with that turn token, a new request ID, your action and optional amount. A raise amount is the total street commitment: raise TO. All-in means the legal maximum wager, or a call when your stack cannot cover it. Reuse the exact request ID/payload only to retry an uncertain submission. After a retry, read `get_table` for current state: the cached receipt records the original move, not later activity. For a stale turn, reread state before deciding again.
